...
function apply_interactively {
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^Protocol.*/Protocol $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^ClientAliveInterval.*/ClientAliveInterval $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^ClientAliveCountMax.*/ClientAliveCountMax $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^PermitEmptyPasswords.*/PermitEmptyPasswords $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^PubkeyAuthentication.*/PubkeyAuthentication $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^PasswordAuthentication.*/PasswordAuthentication $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^AllowGroups.*/AllowGroups $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^Port.*/Port $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
    ...
    if [ -n "$new_value" ]; then
        sed -i "s/^X11Forwarding.*/X11Forwarding $new_value/" /etc/ssh/sshd_config
        confirm_change
    fi
